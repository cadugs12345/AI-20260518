...
print(f"  完成: {len(prices_sorted):,}条")
...
# 用period_dates
print(f"节点数: {len(period_dates)} (pred缓存{len(pred_dates)})")
...
vol_map = {}
for d in period_dates:
    sub = prices_sorted[prices_sorted["trade_date"] == d][["ts_code","vol_60d_ann"]].dropna()
    vol_map[d] = dict(zip(sub["ts_code"], sub["vol_60d_ann"]))

del prices, panel
gc.collect()

# ===== 无成本验证 =====
print(f"\n无成本验证 (20日period_dates):")
for n in [30, 50]:
    rets = []
    for d in period_dates:
...
def backtest_v12(n_stocks, target_vol=0.20, max_leverage=1.0, label=""):
    """
    截面波动率控制：
    1. 每期选出Top N股票
    2. 计算这些股票过去60日波动率的中位数
    3. 仓位 = target_vol / 波动中位数，上限max_leverage
    """
    cash = 0.03
    holdings = {}
    navs = [1.0]
    ...
    for i, date in enumerate(period_dates[:-1]):
        sell_date = period_dates[i+1]
        ...
        if len(selected_vols) >= 5:
            ...
            position_ratio = min(target_vol / median_vol, max_leverage)
...
# ===== 参数扫描 =====
print("\n===== 截面波动控制扫描 =====")
print(f"{'策略':20s} | 年化 | 波动 | 夏普 | 回撤 | 仓位")
print("-"*65)

for n in [30, 50]:
    for tv in [0.15, 0.20, 0.25, 0.30]:
        nav, pnl = backtest_v12(n, target_vol=tv, max_leverage=1.0, 
                                 label=f"ML_Top{n}_目波{tv*100:.0f}")
        print()
