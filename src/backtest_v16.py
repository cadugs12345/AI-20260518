...
# 高波反转 = 波动率 * (-20日动量)
panel["高波反转"] = panel["波动率"] * (-panel["20日动量"])

# 超跌信号 = -RSI_6 * 20日动量  (RSI越低+动量越负→值越大)
panel["超跌信号"] = -panel["RSI_6"] * panel["20日动量"]

# 量价背离 = 量能趋势 - MACD符号调整  (量涨+价跌→值大)
panel["量价背离"] = panel["量能趋势"] - panel["MACD"].abs() * np.sign(panel["MACD"])

# 多排强度 = EMA多头排列强度
ema5 = panel["EMA5偏离"].fillna(0)
ema10 = panel["EMA10偏离"].fillna(0)
ema20 = panel["EMA20偏离"].fillna(0)
panel["多排强度"] = (ema5 + ema10 + ema20)/3 - np.abs(ema5-ema10) - np.abs(ema10-ema20)

# 动量加速 (IC弱，先不加)
# 高波反转的IC最强，纳入

new_factors = ["高波反转", "超跌信号", "量价背离", "多排强度"]
print(f"新因子: {new_factors}")
for f in new_factors:
    print(f"  {f}: sum({panel[f].notna().sum():,}), mean({panel[f].mean():.4f})")

# ===== 因子列 =====
skip_cols = {"ts_code","trade_date","fwd_20d_ret","fwd_5d_ret",
             "均值","20日收益率","短期反转","毛利率","量比","BP","SP","EP","股息率","流通市值"}
factor_cols = [c for c in panel.columns 
               if c not in skip_cols and panel[c].dtype in ("float64","int64")]
...
if os.path.exists(pred_path):
    ...
    for i, date in enumerate(period_dates):
        ...
        sub_train = train[factor_cols + ["fwd_20d_ret"]].dropna()
        train = train.loc[sub_train.index]
        sub_val = val[factor_cols + ["fwd_20d_ret"]].dropna()
        val = val.loc[sub_val.index]
        ...
        if (i+1) % 15 == 0: print(f"  [{i+1}/{len(period_dates)}] train={len(train):,}")
...
def backtest_v16(n_stocks, target_vol=0.15, label=""):
    cash = 0.03
    holdings = {}
    navs = [1.0]
    ...
    for i, date in enumerate(pred_dates[:-1]):
        ...
        if i < 3 or (i+1) % 15 == 0:
            vs = f"{median_vol*100:.0f}%" if not np.isnan(median_vol) else "N/A"
            print(f"  p{i:3d} {str(date.date())}->{str(sell_date.date())} | "
                  f"持{len(holdings):3d} | 股波{vs:5s} | 仓{pos_ratio:.2f} | "
...
for n in [30, 50]:
    for tv in [0.15, 0.20]:
        nav, pnl = backtest_v16(n, target_vol=tv, label=f"v16_Top{n}")
        print()
