...
for i, code in enumerate(all_codes):
    ...
    if factors.empty:
        continue
    # 只取缺失的日期（用字符串匹配避免Timestamp vs str问题）
    date_strs = factors.index.strftime("%Y%m%d")
    keep = date_strs.isin(missing_strs)
    if not keep.any():
        continue
    fy = factors[keep].copy()
    if fy.empty:
        continue
    fy["ts_code"] = code
...
if not ts_new.empty:
    ...
    # 重新完整读取full_prices用于加close列
    full_px = pd.read_parquet(os.path.join(DATA_FACTORS, "full_prices.parquet"),
                                columns=["ts_code","trade_date","close"])
    full_px["trade_date"] = pd.to_datetime(full_px["trade_date"])
    px_missing = full_px[full_px["trade_date"].isin(missing)]
