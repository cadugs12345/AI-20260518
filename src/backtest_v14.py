"""
v14 - 20日调仓 + 截面波动率 + daily_basic财务因子(pe/pb/ps)
基于v3面板（含20日和5日标签，含财务因子）
"""
...
t0 = time.time()
print("="*60)
print("v14 - 20日调仓 + 截面波动率 + pe/pb/ps财务因子")
print("="*60)
...
# ===== 因子列 =====
# 用20日标签
label_col = "fwd_20d_ret"

# 排除不必要的列
exclude = {"ts_code","trade_date","fwd_20d_ret","fwd_5d_ret",
           "pe","pe_ttm","pb","ps","ps_ttm","dv_ratio","dv_ttm",  # 保留！
           "total_mv","circ_mv","市值","流通市值","短期反转","毛利率","量比","BP","SP","EP","股息率","换手率"}
# 把pe/pb等加入因子池
...
pred_path = os.path.join(DATA_FACTORS, "pred_20d_v14.pkl")
...
if os.path.exists(pred_path):
    ...
    print("[ML] 训练预测（20日标签+pe/pb/ps因子）...")
    all_preds = []
    for i, date in enumerate(period_dates):
        ...
        # 清理缺失
        sub = train[factor_cols + [label_col]].dropna()
        train = train.loc[sub.index]
        sub = val[factor_cols + [label_col]].dropna()
        val = val.loc[sub.index]
...
print(f"period_dates: {len(period_dates)} | pred_dates: {len(pred_dates)}")
...
def backtest_v14(n_stocks, target_vol=0.20, label=""):
    cash = 0.03
    holdings = {}
    navs = [1.0]
...
for n in [30, 50]:
    for tv in [0.15, 0.20]:
        nav, pnl = backtest_v14(n, target_vol=tv, label=f"v14_Top{n}")
        print()
