#!/usr/bin/env python3
"""
v36c_repredict_rank_save.py — 用rank标签训练LGB并保存预测
然后check 等权/指数衰减 回测
"""
...
print("加载已保存的rank LGB预测...", flush=True)
preds = pd.read_parquet("data/factors/pred_v37_rank.parquet")
preds["trade_date"] = pd.to_datetime(preds["trade_date"])
print(f"  {len(preds):,}预测, {preds['trade_date'].nunique()}期", flush=True)

# ====== 回测（直接复制v36的backtest_ml函数） ======
def backtest_ml(pred_df, n_stocks=30, target_vol=0.15, label="", min_date=pd.Timestamp("2023-01-01"),
                use_weighted=False, use_industry=False):
    ...
    for i in range(len(pred_dates)-1):
        ...
        sv_l=[sv.get(c,np.nan) for c in sel]; sv_l=[v for v in sv_l if not np.isnan(v) and v>0.01]
...
backtest_ml(preds, 30, 0.15, "v36c rank+等权(同v36引擎)", use_weighted=False, use_industry=False)
backtest_ml(preds, 30, 0.15, "v37a rank+指衰+行业中)", use_weighted=True, use_industry=True)
