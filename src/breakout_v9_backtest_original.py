...
def detect_signals_v9(df):
    """
    v9选股: 涨停→18日线始终向上→跌破再上穿→买入
    """
    ...
    for i in range(n):
        ...
        # 必须回踩到18日线附近且上穿前一两天在均线附近
        if i < 2: continue
        
        # 检查上穿前1~2天是否有在18日线附近（收盘 ≤ 均线×1.02）
        near_condition = False
        for lookback in [1, 2]:
            j = i - lookback
            if j > lb:
                if np.isfinite(ma[j]) and c[j] <= ma[j] * 1.02:
                    near_condition = True
                    break
        if not near_condition: continue
...
def run_backtest():
    ...
    for idx, code in enumerate(codes):
        ...
        sigs_raw = detect_signals_v9(df.iloc[si:].reset_index(drop=True))
        for s in sigs_raw:
            gi = si + s['idx']
    ...
    for tidx, sig in enumerate(all_sigs):
        ...
        # 买入: 次日开盘（不高于18日均线的103%，防止追高）
        do = float(df.iloc[eidx]['open'])
        elv = float(df.iloc[eidx]['low'])
        eclose = float(df.iloc[eidx]['close'])
        ema = sig['ma_value']
        max_entry = ema * 1.03  # 最高不超过18日线3%
        
        ep = min(do, max_entry)
        ep = max(ep, elv)  # 不低于当日最低价
        if ep <= 0: continue
        ...
        for la in range(1, MAX_HOLD+1):
            ...
            row = df.iloc[eidx+la]; ddo, ddh, ddl, ddc = row['open'], row['high'], row['low'], row['close']
            ci = eidx + la
            ...
            # 止盈检查: 触及BOLL上轨
            if ddh >= tp_boll-1e-8:
                epv = ddh; exit_idx=eidx+la; reason='take_profit'; break
            
            # 连续2日跌破18日线止损
            if ddc < stop_price:
                below_count += 1
                if below_count >= MA_BREAK_CANDLES:
                    epv = ddc; exit_idx=eidx+la; reason='stop_loss'; break
            else:
