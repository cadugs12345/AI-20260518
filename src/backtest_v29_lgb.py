"""
v29 LightGBM滚动训练 — 突破夏普1.0
复用v28框架：每半年滚动训练，回测用v27引擎（含摩擦成本/目标波动率）
LightGBM参数: 200棵树, 1%验证集以内部分评估
"""
...
t0 = time.time()
print("="*60)
print("v29 LightGBM滚动训练 — 突破夏普1.0")
...
for hi, (hk, train_cutoff) in enumerate(half_year_dates):
    if hi < 3:  # 前3个半年不训练（2021H1~2022H1），用RF代替
        continue
    ...
    lgb_model = lgb.LGBMRegressor(
        ...
        min_child_weight=10, min_data_in_leaf=50,
    ...
    for d in period_dates:
        ...
        preds = lgb_model.predict(X_te)
        for j, code in enumerate(day["ts_code"].values):
            lgb_preds.append({"trade_date": d, "ts_code": code, "pred_ret": float(preds[j])})
    
    print(f"  {hk} trained: {len(train):,}条 | 预测 {str(d)[:10]}", flush=True)

print(f"  LightGBM预测: {len(lgb_preds):,}条", flush=True)

# ===== 5. 构建回测信号 =====
print("\n[5] 构建回测信号...")

lgb_df = pd.DataFrame(lgb_preds) if lgb_preds else pd.DataFrame(columns=["trade_date","ts_code","pred_ret"])

# 计算IC
print("  计算IC...")
ic_results = []
for d in lgb_df["trade_date"].unique():
    day = lgb_df[lgb_df["trade_date"] == d]
    day_panel = panel[panel["trade_date"] == d]
    merged = day.merge(day_panel[["ts_code","fwd_20d_ret"]], on="ts_code")
    if len(merged) > 10:
        from scipy.stats import spearmanr
        ic, _ = spearmanr(merged["pred_ret"], merged["fwd_20d_ret"])
        ic_results.append(ic)

if ic_results:
    ic_mean = np.mean(ic_results)
    ic_std = np.std(ic_results) if len(ic_results) > 1 else 0.001
    ic_ir = ic_mean / ic_std if ic_std > 0 else 0
    print(f"  LGB IC: {ic_mean*100:+.2f}% | IR: {ic_ir:.2f} | {len(ic_results)}期")

# 构建各策略Top50
pred_records = {"v12": [], "rf": [], "rf_risk": [], "lgb_raw": [], "lgb": []}
...
for i, d in enumerate(period_dates):
    ...
    # LGB
    lgb_day = lgb_df[lgb_df["trade_date"] == d]
    lgb_raw_scores = np.full(len(day), -999.0)
    lgb_fallback_scores = day["s_rf"].values.copy()
    
    if len(lgb_day) > 0:
        lgb_map = dict(zip(lgb_day["ts_code"], lgb_day["pred_ret"]))
        for j, code in enumerate(day["ts_code"].values):
            if code in lgb_map:
                lgb_raw_scores[j] = lgb_map[code]
                lgb_fallback_scores[j] = lgb_map[code]
    
    scores["lgb_raw"] = lgb_raw_scores
    scores["lgb"] = lgb_fallback_scores
    
    # 风控版LGB
    lgb_risk = lgb_fallback_scores.copy()
    for j in range(len(day)):
        if rf_risk[j] == -999:
            lgb_risk[j] = -999
    scores["lgb_risk"] = lgb_risk
    scores["lgb_risk"] = lgb_risk  # 确保存在
    
    for strategy in ["v12", "rf", "rf_risk", "lgb_raw", "lgb", "lgb_risk"]:
        sc = scores[strategy]
        order = np.argsort(-sc)
...
preds = {}
for k in ["v12", "rf", "rf_risk", "lgb_raw", "lgb", "lgb_risk"]:
...
def run_all(ns=30, tv=0.15, label=""):
    ...
    for k, name in [("v12","v12等权"), ("rf","RF"), ("rf_risk","RF+风控"),
                    ("lgb_raw","LGB纯"), ("lgb","LGB+RF回退"), ("lgb_risk","LGB+风控")]:
        if len(preds[k]) > 0:
...
json.dump(result, open("output/backtest_v29_lgb.json", "w"), indent=2, default=str)
...
print("✅ output/backtest_v29_lgb.json")
