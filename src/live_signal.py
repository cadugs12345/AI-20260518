...
t0 = time.time()
print(f"🏆 v31 LGB+指数衰减+行业中性 — {time.strftime('%F %H:%M')}")
print("="*50)
...
lgb_m = lgb.LGBMRegressor(n_estimators=500, max_depth=3, learning_rate=0.02,
...
lgb_m.fit(X_tr[:-n_v], y_tr[:-n_v], eval_set=[(X_tr[-n_v:], y_tr[-n_v:])],
...
pos_df.to_csv(f"{OUTPUT}/v31_positions.csv", index=False,
...
signal = {
    ...
    "version": "v31",
    "config": {"n_hold": n_hold, "weighting": "指数衰减(e^-0.1r)", "industry_neutral": "每行业最多3只"},
    "model": "LightGBM 79因子 (夏普1.25)",
    "lgb_best_iter": lgb_m.best_iteration_,
    "positions": positions
}
json.dump(signal, open(f"{OUTPUT}/v31_signal.json","w"), indent=2)

# 保存模型
joblib.dump({"model": lgb_m, "factor_cols": factor_cols}, "models/live_lgb_model.joblib")
...
print(f"\n📊 {latest_date} v31 Top30 (指数衰减+行业中性):")
